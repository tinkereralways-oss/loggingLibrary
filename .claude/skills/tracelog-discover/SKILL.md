---
name: tracelog-discover
description: Analyze a microservice codebase and produce a trace-log integration plan identifying auto-instrumented inflows, required code changes, thread handoffs, and outbound propagation points
disable-model-invocation: true
argument-hint: "[service-path (optional, defaults to current directory)]"
---
...
Now analyze the microservice codebase at $ARGUMENTS (or the current directory if not specified) and produce a trace-log integration plan. For each section, cite specific files, classes, and line numbers from the target repo.
...
Show the exact dependency to add to this project's build file (Maven or Gradle, matching what the project uses).
